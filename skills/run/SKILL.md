...
2. **For each phase, in order:**
   ...
   b. **Execute.** Loop: spawn a `goal-worker` (Task) with PLAN.md + the current `[FAIL]` criteria; then `gdcc check`. Repeat until the phase's **exit condition** holds (its target criteria are `[PASS]`). If a worker dies from a transient API error, spawn another (POLICY: up to 3). Workers are subagents with their own context, so your master context grows slowly.
      - **Dispatch synchronously — this is the default and what the Stop gate assumes.** Spawn each worker as a **blocking** Task (`run_in_background: false`) and wait for it to return before `gdcc check`. Do NOT background the worker: if you yield the turn while it churns, the Stop gate fires on a torn (half-written) scoreboard and would push you to re-dispatch a **second** worker onto the same files. Background dispatch is only for the quota waits/watch below.
      - **If you must background a worker** (e.g. a very long build/benchmark you want to leave the master free during), bracket it so the gate can tell "still churning" from "quitting": run `gdcc worker begin <id>` right before you background it, and `gdcc worker end <id>` the moment it returns (on the completion notification). While a marker is live the Stop gate **defers** (lets you rest until the worker wakes you) instead of blocking or re-dispatching; a crashed worker's marker expires on its own (`GDCC_WORKER_TTL`).
